...
MiniFlow 的节点可以执行 JS、发起任意 HTTP 请求，因此**它等价于一台可编程服务器**。请务必注意：

1. **默认只监听 `127.0.0.1`**。`HOST=0.0.0.0` 会把服务暴露到局域网，任何人都能创建工作流并执行任意代码。
2. **不要暴露 `$env`**。表达式里的 `$env` 只返回 `MINIFLOW_EXPOSED_ENV` 白名单中列出的变量（默认为空），
   这是为了防止 API Key 被写进提示词或落进执行日志。密钥请放在服务端环境变量里，由节点自行读取。
3. **多租户场景请开启 `MINIFLOW_SAFE_MODE=1`**，这会禁用 `code` 节点和所有 `{{ }}` 表达式求值。
4. **`.env` 已在 `.gitignore` 中**。提交前请确认没有把真实密钥写进仓库；
   如果发生过泄露，光覆盖文件是不够的 —— 去厂商后台**吊销并重新生成**该密钥。
5. Webhook 端点没有鉴权。若要对公网开放，请在前面加一层反向代理鉴权。
